...
class HeartRate:
    def __init__(self, age, gender, weight, filename):
        ...
        for hours in range(0, hourNum + 1):
            r = open("hour" + str(hours) + ".csv", 'r') as csvfile:
                csv_r = csv.reader(r)
            counter = 0
            splitter = 0
            print(str(hours))
            ...
            for elem in range(0, len(hrTime)):
                timeValue2[hrTime[elem]] = hrValue[elem]

            for elems in range(0, len(hrTime)-1):
                t1 = hrTime[counter]
                t2 = hrTime[counter + 1]
                counter += 1
                ...
                if (int(dt2.timestamp() - dt1.timestamp()) <= interval):
                    with open("hour" + str(hours) + "_" + str(splitter) + ".csv", 'a', newline = "") as csvfile:
                        filewriter = csv.writer(csvfile, delimiter=',', quotechar="|", quoting=csv.QUOTE_MINIMAL)
                        filewriter.writerow([timeValue2[hrTime[counter+1]]])
                else:
                    splitter += 1
